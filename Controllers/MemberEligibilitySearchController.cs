...
namespace ANGDDEAPI.Controllers
{
    ...
    public class MemberEligibilitySearchController : ControllerBase
    {
        ...
        public MemberEligibilitySearchController(IHttpContextAccessor httpContextAccessor, DOConfiguration objConfiguration, IMemoryCacheHelper memoryCacheHelper, BOCommon bOCommon, ICacheService cache)
        {
            _cache = cache;
        ...
        public ActionResult GetMemberSearchDetailsFromGPSnCSP_Restriction(string details = null)
        {
                ...
                if (!string.IsNullOrEmpty(details))
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(query))
                    {
                        ...
                        if (memberSearchDetails == null || memberSearchDetails.Count == 0)
                        {
                            ...
                            if (!string.IsNullOrEmpty(memberDetails.groupId) && !string.IsNullOrEmpty(memberDetails.medicaidId))
                            {
            ...
            {
                //BOCommon.Trace("Exception-MembersearchController-GetMemberSearchDetailsFromGPSnCSP_Restriction", "", ex.StackTrace, "4");
                if (source.Contains("DDE"))
                {
                    _objBOCommon.ErrorLog(LoggedInUserId, this.GetType().Name + System.Reflection.MethodBase.GetCurrentMethod(), ex.Message, ex.ToString());
                }
        ...
        private string GetPlanSearchQuery(DOMemberSearchCriteria planDetails)
        {
            ...
            try
            {
                ...
                if (planDetails != null)
                {
                    ...
                    {
                        planSearchQueryBuilder.Append(ConstantTexts.GPSPlanSelectScript1LIS);///oracle query string
                    }
                    else
                    {
        ...
        [HttpGet]
        public ActionResult GetRemarkCodeDetails(string MemberId = null)
        {
            DOMemberSearchCriteria planDetails = null;
            List<DOMemberPlanDetails> planSearchDetails = null;
            source = BOCommon.GetRefererURI(Request);
            try
            {
                ...
                if (!string.IsNullOrEmpty(MemberId))
                {
                    ...
                    planDetails = new DOMemberSearchCriteria();
                    planDetails.MemberId = MemberId;
                    //planDetails = JsonConvert.DeserializeObject<DOMemberSearchCriteria>(MemberId);
                    //if (planDetails != null)
                    //{
                    //    LoggedInUserId = planDetails.UserId;
                    //}
                    ...
                    if (!string.IsNullOrEmpty(planDetails.MemberId))
                    {
                        objDOCSPEligMemberCoverageRequest.subscriberId = MemberId;
                        objDOCSPEligMemberCoverageRequest.dateOfServiceFrom = "2016-01-01";
                        objDOCSPEligMemberCoverageRequest.dateOfServiceTo = DateTime.Today.ToString("yyyy-MM-dd");
                    }

                    objCSPEligibilityMethods.CSPMemberCoverageDetailsforRemarkCode(objDOCSPEligMemberCoverageRequest, out planSearchDetails);
                }
                return Ok(planSearchDetails);
            }
            catch (Exception ex)
        ...
        public ActionResult GetMemberSearchDetailsByPolicyID(string exchPolicyId = null)
        {
            List<DOGPSMemberDetails> memberSearchDetails = null;
            ...
            {
                _username = _httpContextAccessor.HttpContext.User.Identity.Name;
                 DOMemberDetailsResponseByPolicyId MemberDetailsResponse  = new DOMemberDetailsResponseByPolicyId();
                if (!string.IsNullOrEmpty(exchPolicyId))
                {
                    CSPEligibilityMethods objCSPEligibilityMethods = new CSPEligibilityMethods(_memoryCacheHelper, _objConfiguration, objAPICall);
                    objCSPEligibilityMethods.GetSubscriberIdByPolicyId(exchPolicyId, out MemberDetailsResponse);

                    if (MemberDetailsResponse != null && MemberDetailsResponse.data != null)
                    {
                        exceptionResult = objCSPEligibilityMethods.MapToMemberPolocyDetails(MemberDetailsResponse, out memberSearchDetails);
                    }
